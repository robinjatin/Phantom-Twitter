...
class MyProfile extends Component {
    ...
    handleShow2 = (e, userid) => {
        this.setState({show2:true,userId:userid});
    }
    ...
    async componentDidMount() {
        ...
        const posts = this.state.tweets.filter(item => item.user === localStorage.getItem("user"));
    ...
    handleAccountDelete = (e, user) => {
        ...
        axios.delete("http://localhost:8082/deleteuser/" + user).then(res =>
        this.props.history.push("/")).catch((error) => {
            console.log("Could not delete the account, error: ", error)
        });
        }
    }
    handleLogout = () => {
            ...
            <bootstrap.Container>
                ...
                <div style={{display: "flex",justifyContent: "center", padding:"28px"}}>
                        <Card border="primary" className="bg-dark text-white" >
                            ...
                            <EditDetails userid={this.state.userId} handleSubmission={this.handleClose2}/>}
                            
                                </Card.Text>
                                {this.state.show2 ? "" :
                                <div>
                                <Button onClick={e => this.handleShow2(e, this.state.user.user)} variant="secondary" >
                                    <span>Edit Details</span>
                                </Button>
                                <Button onClick={e => this.handleAccountDelete(e, this.state.user.user)} variant="danger" className="topnav-right">
                ...
                {this.state.tweets.length === 0 ? (
                    ...
                    {this.state.tweets.map(tweet => (
                        <bootstrap.Row style={{border: "2px solid white", padding:"22px"}} key={tweet._id} className="m-4 p-4 inline-flex border-2 border-black bg-gray-400" width="400">
                            ...
                            <bootstrap.Row><bootstrap.Col xs={14}> <p style={{marginLeft: "-45px"}}><strong>{this.state.user.name}</strong> &nbsp;  @{this.state.user.user} &nbsp;·&nbsp;  {tweet.createdAt}</p></bootstrap.Col><bootstrap.Col><bootstrap.Dropdown>
                                    <bootstrap.Dropdown.Toggle variant="secondary" size="sm" className="float-center" id="dropdown-basic">
